...
<form id="edit-test" action="<c:url value="pi/examen/edition"/>" method="POST" class="form--horizontal">
    ...
    <div class="modal__mod--sm modal__mod--vertical">
        ...
        <input type="hidden" name="teacher" class="form--control" value="<c:out value='${ sessionScope.sessionProfesseur.id }'/>" pattern=".{1,55}" readonly="readonly" required/>
        <label>FORMAT</label>
        ...
        <input type="text" name="title" class="form--control" value="<c:out value="${ examen.nom }"/>" pattern=".{5,55}" placeholder="Nom" x-moz-errormessage="Veuillez entrer un nom correct" required/>
        <span class="form__error">${ form.erreurs['title'] }</span>
        ...
        <label>GROUPE</label>
        <input type="text" name="group" class="form--control" value="<c:out value="${ examen.groupe.nom}"/>" pattern="G[a-zA-Z]{1}" disabled readonly required/>
        ...
        <span class="form__error">${ form.erreurs['matter'] }</span>
    </div>
        ...
        <table>
            ...
            <c:forEach items="${examen.listeNotes}" var="note">
                <tr class="tr--md">
                    ...
                    <td>
                        <input type="hidden" name="students[]" value="<c:out value="${note.etudiant.id}"/>" readonly required/>
                        <input type="text" name="scores[]" value="<fmt:formatNumber type="number" maxFractionDigits="1" value="${note.note}" />" placeholder="Note" pattern="[0-9,.]{1,5}" x-moz-errormessage="Veuillez entrer un nombre avec ou sans virgule">        
                    </td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
        <span class="form__error">${ form.erreurs['scores'] }</span>
    </div>          
