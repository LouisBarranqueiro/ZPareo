...
<form id="delete-student" action="<c:url value="ai/etudiant/suppression"/>" method="POST" class="form--horizontal">
    ...
    <div class="modal__mod--lg">
    	<p>Êtes-vous sûr de vouloir supprimer l'étudiant : <c:out value="${etudiant.prenom}"/> <c:out value="${etudiant.nom}"/> ?</p>
    	<input type="hidden" name="id" value="<c:out value='${ etudiant.id }'/>" size="30" pattern="[0-9]{1,11}" readonly required/>             
    </div>
